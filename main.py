...
# 1. LOGGING ON (Taaki terminal mein sab dikhe)
...
# 2. CLIENT CONFIG (Same as debug.py)
client = Client(
    "bot_session", 
    api_id=API_ID, 
    api_hash=API_HASH, 
    bot_token=BOT_TOKEN, 
    ipv6=False  # YE line sabse zaruri hai
)
...
async def spy(c, m: Message):
    logger.info(f"👀 MSG: {m.text} | FROM: {m.from_user.id}")
...
async def start_cmd(c, m: Message):
    if m.from_user.id != OWNER_ID:
        ...
        return
    await m.reply_text("✅ **Bot is Online & Connected!**")

@client.on_message(filters.command("check"))
async def check_cmd(c, m: Message):
    await m.reply_text("🚀 **System All Green!**")
...
async def media_handler(c, m: Message):
    ...
    msg_id = m.forward_from_message_id if m.forward_from_chat else None
    title = m.caption or m.video.file_name or f"Video {msg_id}"
    
    if msg_id and m.forward_from_chat.id == CHANNEL_ID:
